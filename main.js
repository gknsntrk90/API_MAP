...
function renderMarker(item) {
  ...
  // Markerı oluşturur
  L.marker(item.coords, { icon: detecIcon(item.status) }) //*bir değişken tanımla.
...
function renderNoteList(item) {
  ...
  list.innerHTML = "";
  layerGroup.clearLayers(); //* markerları temizlemek için bu kodu kullanıyoruz
  //* Markerlerı temizler
...
function handleClick (e) {
    ...
    if(e.target.id === "delete"){
        //* id'sini bildiğimniz elemanı diziden filter yöntemi kullanarak kaldırdık
        notes = notes.filter((note) => note.id != id);
        setStorage(notes); //* localStorage'a güncelle
        renderNoteList(notes); //* Ekranı güncelle
    }
    if (e.target.id == "fly"){
       const note =  notes.find((note) => (note.id) == id); //*Tıkladığımız elemanın id'si ile dizi içerisindeki elemanlardan herhangi birinin id'si eşleşirse bul.
